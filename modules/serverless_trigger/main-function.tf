...
resource "local_file" "container_config" {
  filename                 = "${local.function_path}/container_config.json"
  content                  = jsonencode(var.container_resources)
}
...
resource "yandex_function" "restart_container" {
  description              = "Function to restart container via  SDK"
