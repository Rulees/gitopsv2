...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ...
    def _init_client(self, config):
        ...
        if not work_dir:
            raise AnsibleError("❌ WORK_DIR не установлен. Укажи его в окружении!")
        ...
        if not token and not os.path.isfile(sa_key_path):
            raise AnsibleError(
                f"❌ Не найден ни OAUTH_TOKEN в окружении, ни SA ключ по пути: {sa_key_path}\n"
                f"💡 Укажи хотя бы один из них."
    )
    ...
    def parse(self, inventory, loader, config_path, cache=True):
        ...
        if not os.path.isfile(config_path):
            raise AnsibleError("❌ Конфиг yc_compute.yml не найден в ожидаемых локациях!")
